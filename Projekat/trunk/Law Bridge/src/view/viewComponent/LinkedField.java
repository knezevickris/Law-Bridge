...
public class LinkedField extends JPanel implements InputField
{
	...
	public Object getValue() {
		...
		try {
	        ...
	        Connection conn = instance.getConn();
			ResultSet pks;
			pks = conn.getMetaData().getPrimaryKeys(instance.getDatabase(), "pisg2", refTable);
			
			
			ResultSet fks = conn.getMetaData().getImportedKeys(instance.getDatabase(), "pisg2", refTable);
			boolean isFk = true;
			...
			String query = "SELECT ["+ refTable +"].[" + colName + "] FROM [" + refTable + "] WHERE [" + refTable + "].[" + refColumn + "]='" + field.getValue() + "'";
